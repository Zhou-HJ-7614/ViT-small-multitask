...
v3 改进:
  - Copy-Paste 数据增强: 50% 概率随机粘贴 Donor 前景像素，打破背景共现偏差
...
class VOCMultiTaskDataset(Dataset):
    ...
    def __getitem__(self, idx: int):
        ...
        # ==================== Copy-Paste 核心逻辑 ====================
        # 仅在训练阶段 (有 transform) 且非纯背景图时以一定概率触发
        if self.transform is not None and not is_empty and random.random() < self.copy_paste_prob:
            # 1. 随机挑选另一张原图作为素材提供者 (Donor)
            donor_idx = random.randint(0, self.num_normal - 1)
            donor_image, donor_mask, _ = self._load_image_and_mask(donor_idx)

            # 2. 核心问题修复: VOC 图片尺寸不一，先将 Donor 强行 Resize 到当前图片的尺寸
            w, h = image.size
            donor_image = donor_image.resize((w, h), Image.BILINEAR)
            # mask 必须用 NEAREST，防止插值产生非法类别值
            donor_mask_img = Image.fromarray(donor_mask).resize((w, h), Image.NEAREST)
            
            donor_img_np = np.array(donor_image)
            donor_mask_np = np.array(donor_mask_img)
            img_np = np.array(image)

            # 3. 找出 Donor 中所有的真实前景物体 (类别 1~20)
            fg_indices = (donor_mask_np > 0) & (donor_mask_np < 255)

            if fg_indices.any():
                # 4. 像素覆盖 (Copy & Paste)
                img_np[fg_indices] = donor_img_np[fg_indices]
                mask[fg_indices] = donor_mask_np[fg_indices]
                
                # 更新 image 为覆盖后的新图
                image = Image.fromarray(img_np)
        # ======================================================================

        # 因为 mask 已经被 Copy-Paste 改变，现在生成的 labels 绝对是正确的
...
class MultiTaskLoss(nn.Module):
    ...
    def __init__(self, cfg: Config):
        ...
        self.criterion_seg = SegmentationLoss(num_classes=21, dice_weight=1.0, boundary_weight=0.5)
...
def evaluate(model: nn.Module, data_loader: DataLoader,
    ...
    criterion_seg = SegmentationLoss(num_classes=21, dice_weight=1.0, boundary_weight=0.5)
    total_loss = 0.0
...
def train(cfg: Config = None):
    ...
    train_dataset = VOCMultiTaskDataset(
        ...
        copy_paste_prob=0.5)  # 激活 Copy-Paste (50% 概率)
